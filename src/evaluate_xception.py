...
# paths
MODEL_PATH = "/content/drive/MyDrive/deepfake_detection/xception_best.pth"
...
for inputs, labels in tqdm(test_loader):
    ...
    for i in range(len(labels)):
        ...
        if label == pred and len(correct_samples) < 10:
            ...
            cam = generate_gradcam(model, inputs[i].detach().clone(), pred, model.conv1, device)
...
df.to_csv(os.path.join(SAVE_DIR, "metrics_xception.csv"), index=False)
...
plt.ylabel("True")
plt.savefig(os.path.join(SAVE_DIR, "confusion_matrix_xception.png"))
